...
export const useTasksHandlers = (
  ...
  const handleAddTaskClick = useCallback(
    (event: KeyboardEvent<HTMLInputElement>) => {
      if (event.key === "Enter") {
        const newTask: ITask = { id: String(Math.random()), label: todoCreationInputValue, status: "Active" };
  ...
  const handleChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    const updateTasks: ITask[] = tasks.map((task) => {
      if (task.id === id) return { ...task, status: task.status === "Active" ? "Completed" : "Active" };
      return task;
    });
